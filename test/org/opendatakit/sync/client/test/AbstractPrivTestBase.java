...
public abstract class AbstractPrivTestBase extends TestCase {
	...
	abstract SyncClient createNewSyncPrivClient();


	abstract SyncClient createNewAdminPrivClient();
	...
	protected void setUp() throws Exception {
	   ...
	   SyncClient adminPrivClient = createNewAdminPrivClient();
	   
	   // Delete all files on the server 
	   JSONObject appFiles = adminPrivClient.getManifestForAppLevelFiles(agg_url, appId, version);

	   JSONArray files = appFiles.getJSONArray(SyncClient.FILES_STR);
	    
	   for (int j = 0; j < files.size(); j++) {
	       adminPrivClient.deleteFile(agg_url, appId, files.getJSONObject(j).getString(SyncClient.FILENAME_STR), version);
	   }
	    
	   // Delete all tables on the server 
	   JSONObject tablesObj = adminPrivClient.getTables(agg_url, appId);

	   JSONArray tables = tablesObj.getJSONArray(SyncClient.TABLES_JSON);
	    
	   for (int i = 0; i < tables.size(); i++) {
	       adminPrivClient.deleteTableDefinition(agg_url, appId, tables.getJSONObject(i).getString(SyncClient.TABLE_ID_JSON), tables.getJSONObject(i).getString(SyncClient.SCHEMA_ETAG_JSON));
	   }
	...
	void verifyAuthenticationFailure(int responsecode, SyncClient syncPrivClient) {
	...
	private boolean checkThatFileExistsOnServer(String agg_url, String appId, String relativeFileNameOnServer) {
		...
		SyncClient syncPrivClient = null;
		try {
	...
	public void testDownloadFileWithValidBinaryFile_ExpectFail() {
		...
		SyncClient adminPrivClient = null;
		SyncClient syncPrivClient = null;
	...
	public void testGetManifestForTableIdWhenTableFileExists_ExpectPass() {
	   ...
	   String testFile = absolutePathOfTestFiles + testTableId + File.separator + "spaceNeedle_CCLicense_goCardUSA.jpg";
		...
		try {
	      ...
	      // Make sure that the file is on the server
	      foundFile = checkThatTableLevelFileExistsOnServer(agg_url, appId, testTableId, relativeTestFilePath);
	      
	      assertTrue(foundFile);

			JSONObject obj = syncPrivClient.getManifestForTableId(agg_url, appId, testTableId, version);

	      // Make sure there are no files returned
	      JSONArray files = obj.getJSONArray("files");
	      int numOfFiles = files.size();
	      
	      assertEquals(numOfFiles, 1);
	...
	public void testCreateRowsUsingJSONBulkUpload_ExpectPass() {
		...
		try {
			...
			if (tableDef.containsKey(SyncClient.ORDERED_COLUMNS_DEF)) {
				...
				assertEquals("Date_and_Time", col.getString(SyncClient.ELEM_KEY_JSON));
			}
			...
			for (int i = 0; i < testSize; i++) {
				...
				tempRow.put(SyncClient.ID_JSON, Integer.toString(i));
	...
	public void testUpdateRowWhenRowExists_ExpectPass() {
		...
		try {
			...
			dkvl.add(dkv);
			Row rowObj = Row.forUpdate(row.getRowId(), jsonRow.getString(SyncClient.ROW_ETAG_JSON), row.getFormId(),
					jsonRow.getString(SyncClient.LOCALE_JSON), jsonRow.getString(SyncClient.SAVEPOINT_TYPE_JSON),
					jsonRow.getString(SyncClient.SAVEPOINT_TIMESTAMP_JSON),
					jsonRow.getString(SyncClient.SAVEPOINT_CREATOR_JSON), row.getRowFilterScope(), dkvl);
	...
	public void testBatchGetFileForRowWithValidBinaryFiles_ExpectPass() {
		...
		String pathToVerify = pathToSaveFile + SyncClient.SEPARATOR_STR + relativePathOnServer;
