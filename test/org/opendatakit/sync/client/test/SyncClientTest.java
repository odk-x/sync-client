...
public class SyncClientTest extends TestCase {
  ...
  protected void setUp() throws Exception {
    ...
    for (int j = 0; j < files.size(); j++) {
      wc.deleteFile(agg_url, appId, files.getJSONObject(j).getString(SyncClient.FILENAME_STR), version);
    }
    ...
    for (int i = 0; i < tables.size(); i++) {
      wc.deleteTableDefinition(agg_url, appId, tables.getJSONObject(i).getString(SyncClient.TABLE_ID_JSON), tables.getJSONObject(i).getString(SyncClient.SCHEMA_ETAG_JSON));
    }
  ...
  public boolean checkThatFileExistsOnServer(SyncClient wc, String agg_url, String appId, String relativeFileNameOnServer) {
  ...
  public boolean checkThatTableLevelFileExistsOnServer(SyncClient wc, String agg_url, String appId, String tableId, String relativeFileNameOnServer) {
	    // Make sure the server has added the file
	    boolean found = false;
  ...
  public boolean checkThatInstanceFileExistsOnServer(SyncClient wc, String agg_url, String appId, String tableId,
...
//  }
  ...
  public void testGetManifestForTableIdWhenTableFileExists_ExpectPass() {
    ...
    try {
      ...
      JSONObject obj = wc.getManifestForTableId(agg_url, appId, testTableId, version);
...
//  }
  ...
  public void testUpdateRowWhenRowExists_ExpectPass() {
    ...
    try {
      ...
      dkvl.add(dkv);
      Row rowObj = Row.forUpdate(row.getRowId(), jsonRow.getString(SyncClient.ROW_ETAG_JSON), row.getFormId(), jsonRow.getString(SyncClient.LOCALE_JSON), jsonRow.getString(SyncClient.SAVEPOINT_TYPE_JSON), 
          jsonRow.getString(SyncClient.SAVEPOINT_TIMESTAMP_JSON), jsonRow.getString(SyncClient.SAVEPOINT_CREATOR_JSON), row.getRowFilterScope(), dkvl);
